...
from .impedance import calculate_impedance, MeasurementStrategy, TestFrequency
...
class XDAQ:
    ...
    def __init__(self, debug: bool = False, dev: Board = None):
        if dev is not None:
            self.dev = dev
        else:
    ...
    def config_fpga(self, rhs: bool = False, bitfile: str = None) -> Tuple[int, int]:
        if bitfile is None:
            bitfile = resources.rhs.bitfile_path if rhs else resources.rhd.bitfile_path
    ...
    def measure_impedance(
        ...
        measurement_strategy: MeasurementStrategy = MeasurementStrategy.auto(),
        channels: List[int] = None,
        progress: bool = True,
        raw_measurement_only: bool = False,
        ...
        When raw_measurement_only is True:
        raw_data: np.ndarray
        """
        for i in range(8):
            self.enableExternalDigOut(i, False)
        ...
        period = test_frequency.get_period(sample_rate)
        test_frequency = test_frequency.get_actual(sample_rate)

        reg = self.getreg(self.sampleRate)
        cmd = reg.createCommandListZcheckDac(test_frequency, 128)
        self.uploadCommandList(cmd, 0, 1)
        self.selectAuxCommandLength(0, 0, len(cmd) - 1)
        self.selectAuxCommandBank('all', 0, 1)
        num_periods = measurement_strategy.get_num_periods(test_frequency)
