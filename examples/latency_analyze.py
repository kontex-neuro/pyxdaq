...
parser.add_argument(
    '--spike-us', type=float, default=None,
...
pct = {p: np.percentile(data, p) for p in (50, 95, 99, 99.9, 99.99)}
...
print(f'  min={data.min()} p50={pct[50]:.0f} p95={pct[95]:.0f} p99={pct[99]:.0f} '
      f'p99.9={pct[99.9]:.0f} max={data.max()} us')
print(f'  >{spike_us:.0f} us{"" if args.spike_us is not None else " (p99 x 1.5)"}: '
      f'{spikes.sum()} ({100 * spikes.mean():.3f}%)')
print(f'  ~{samples_per_chunk:.1f} samples/chunk -> sawtooth span '
...
if recorded_chunk is not None:
    ...
    print(f'  recorded {rec_median:.1f} median, {recorded_chunk.mean():.1f} mean '
...
for name, v in stages.items():
    ...
    if len(v):
        print(f'  {name:<10s} p50={np.percentile(v, 50):8.1f}  '
...
for i, b in enumerate(np.flatnonzero(np.diff(z_) > 0) + 1):
    a.axvline(
        b, color='#e53e3e', lw=.8, ls='--', alpha=.7,
        label='new chunk delivered' if i == 0 else None
    )
a.set(
    xlabel='sample index', ylabel='latency (us)',
...
roll = {
    p: np.array([np.percentile(data[s:e], p) for s, e in zip(edges[:-1], edges[1:]) if e > s])
    for p in (50, 95, 99)
}
...
b.plot(
    t[spikes], data[spikes], '.', color='#e53e3e', ms=4, alpha=.6,
    label=f'>{spike_us:.0f} us (n={spikes.sum()}, {100 * spikes.mean():.3f}%)'
)
b.set(xlabel='time (s)', ylabel='latency (us)', yscale='log',
...
c.set(
    xlabel='latency (us)', ylabel='count',
...
d_.set(
    xlabel='latency (us)', ylabel='P(latency > x)', xscale='log', yscale='log',
...
config = ''
if meta:
    config = (f"   |   {'RHS' if meta.get('rhs') else 'RHD'} "
